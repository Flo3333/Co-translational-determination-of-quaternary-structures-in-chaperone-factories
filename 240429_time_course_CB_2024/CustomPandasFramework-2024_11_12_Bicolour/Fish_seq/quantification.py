...
def spots_quantif(rna1_clusteredspots_dataframe: pd.DataFrame,
                  rna2_clusteredspots_dataframe: pd.DataFrame,
                  suntag_clusteredspots_dataframe: pd.DataFrame,
    ...
    dataframes = {
        'rna1' : rna1_clusteredspots_dataframe,
        'rna2' : rna2_clusteredspots_dataframe,
        'suntag' : suntag_clusteredspots_dataframe,
    }
    signal = {
        'rna1' : rna1_signal,
        'rna2' : rna2_signal,
        'suntag' : suntag_signal,
    }
    
    Spots = pd.DataFrame()

    for key in dataframes.keys() :
        if len(dataframes[key]) > 0 :
            Z,Y,X = zip(*rna1_clusteredspots_dataframe.loc[:,['z','y','x']].to_numpy())
            dataframes[key]['spot_type'] = key
            dataframes[key]['intensity'] = signal[key][Z,Y,X]
            dataframes[key]['is_clustered'] = ~dataframes[key]['cluster_id'].isna()
            dataframes[key]['cell_id'] = cell_label[Y,X]
            dataframes[key]['in_nucleus'] = nucleus_label[Y,X].astype(bool)
            dataframes[key] = dataframes[key].loc[np.isin(cell_label[Y,X], fov_cell["cell_id"])] #Keep only spots in quantified cells. This step has to be last to ensure list order conservation
    Spots = pd.concat(
        [*dataframes.values()],
        axis=0
    )
...
def cell_quantif(
    ...
    colloc_keys = ['rna_coord'] + list(colloc_obj.keys())
...
def cluster_quantif(
        cluster_dict : 'dict[pd.DataFrame]'
) :
    for key in cluster_dict.keys() :
        cluster_dict[key]['channel'] = key
    
    Clusters: pd.DataFrame = pd.concat([cluster_dict.values()], axis=0)
...
def _keep_spots_in_mask(spots,mask:np.ndarray) :
    ...
    if mask.ndim == 2 and len(spots[0]) >= 3 :
        spots_indexer = spots_indexer[1:3]
    elif mask.ndim == 3 and len(spots[0]) >= 3 :
        spots_indexer = spots_indexer[:3]
...
def _cell_coloc_quantification(cell:dict, colocalisation_distance, voxel_size, z_shape, object_keys: 'list[str]') :
    
    #Empty spots list are handled in measure.spots_colocalisation and will return nan.
    
    cell_id = cell['cell_id']
    cell_mask = cell['cell_mask']
    objects = {key : cell[key] for key in object_keys}
    print(object_keys)
    ...
    object_number = {
        key : 0 if type(objects[key]) == type(None) else len(objects[key])
    for key in object_keys}

    measures = {'cell_id' : [cell['cell_id']]}
    measures.update({
        key : [object_number[key]] for key in object_keys
    })
    ...
    for object1, object2 in list(itertools.combinations(objects.keys(), 2)) : # give all the combination of keys of length 2 in a list; all elements are a tuple such as ('rna1', 'rna2')
        ...
        if object1 == 'cell_id' or object2 == 'cell_id': continue
