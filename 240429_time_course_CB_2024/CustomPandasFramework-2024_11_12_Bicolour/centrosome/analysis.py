...
import CustomPandasFramework.pvalues as pvalue
...
def individual_violin_plots(table_path, output_path, measures, rna_list = None, folder_name = 'individual_violin_plots/', frameon=True, ext='.svg') :
    """
    ...
    Cell = Cell.loc[Cell['treatment'] != 'CB16h']
    ...
    for column in measures :
        ...
        for rna in data.index.get_level_values(0).unique() :
            ...
            ax = plt.gca()
            violin.violin_plot(ax, data_rna, labels= labels, colors=colors, ylabel= column, title= "{0}\n{1}".format(rna, column), showmeans=True)

            plt.savefig(output_path + "{0}/{1}_{0}{2}".format(column, rna,ext))
            plt.close()
...
def overall_plot(table_path, output_path, measures, rna_list=None, folder_name = 'overall_plots', show_cell_numbers=True, frameon=True, ext='.svg', **kargs) :
    """
    Overal violin plots
    """
    ...
    Cell = Cell.loc[Cell['treatment'].isin(['DMSO','inh216h'])]
    print(Cell.value_counts(subset=['rna','treatment']))
    ...
    for measure in measures :
        ...
        plt.savefig(path + measure + ext)
        plt.close()

def cell_distribution(table_path, output_path, measure_list, rna_list=None, frameon=True, ext='.png') :
    if not ext.startswith('.') : ext = '.' +ext
    ...
    for measure in measure_list :
        ...
        for rna in Cell.index.get_level_values(0).unique() :
            ...
            plt.savefig(path + "{0}_cell_distribution{1}".format(rna,ext))
            plt.close()
...
def Tukey_hsd_tiles(table_path, output_path, measure_list, rna_list, significance= 0.01, folder_name= 'Tukey_tiles', ext='.png', **kwargs) :
    if not folder_name.endswith('/') : folder_name += '/'
    if not ext.startswith('.') : ext = '.' + ext
    path = output_path + '{0}'.format(folder_name)
    os.makedirs(path, exist_ok= True)
    ...
    for measure in measure_list :
        ...
        for rna in rna_list :
            ...
            plt.savefig(path + '{0}_{1}_Tukey_Tile{2}'.format(measure, rna,ext))
            plt.close()
...
def extract_pvalue(
    ...
    """
    Function to extract pvalues in spreadsheet to transmit to partner team. Easier for them to use than graph.
    """
    
    cell_df = prepare_data_frame(intput_path,group_keys + measure_keys, index_keys=[], rna_list=rna_list)

    pvalue_df = pvalue.compute_N_treatment_pvalue(
        cell_df,
        group_keys=group_keys,
        measure_keys=measure_keys
    )

    os.makedirs(output_path + "/pvalue_spreadsheet/", exist_ok=True)
    pvalue_df.to_excel(output_path + "/pvalue_spreadsheet/{0}pvalue.xlsx".format(extract_name))
