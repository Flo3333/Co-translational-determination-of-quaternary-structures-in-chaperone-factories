...
PATH_IN= "/media/floricslimani/SSD 4To/SSD_floricslimani/Fish_seq/Manon/2403_plate_FISH_DNA_HeLa_Kyoto/merge_folder/"
PATH_OUT= "/media/floricslimani/SSD 4To/SSD_floricslimani/Fish_seq/Manon/2403_plate_FISH_DNA_HeLa_Kyoto/analysis/"
...
if DO_STEP3 :
    ...
    data = Cell.groupby(['rna1','treatment'])['rna1_clustered_number'].apply(list).rename('single molecule count in foci')
    fig = plt.figure(figsize= FIGSIZE)
    ax = fig.gca()
    ax = plot.distribution_super_plot(
        data = data,
        ax=ax,
        title= "Number of Cy3 single molecule in foci per cell",
        xlabel= 'RNA',
        ylabel= 'single molecule count'
    )

    fig.savefig(PATH_OUT + '/distributions/Cy3_in_foci_count_per_cell.png')
    plt.close()
